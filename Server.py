...
from Utils import Utils, Global, logs, loop
...
class Server:
    ...
    def __init__(self, key_api, encode_key, port=6060):
    ...
    def routes(self):
        return [
            ...
            web.post('/action/add', self.add_akk),
    ...
    def check_server_params(self, params: dict, method: str, login: int):
        if params:
    ...
    async def do_action(self, data: Data):
        ...
        self.check_server_params(params, method, login)
        ...
        loop.add(self.server_task(name, func, params), timer)
    ...
    async def get_loop_tasks(self, data: Data):
        ...
        for task in loop.task:
            ...
            if isinstance(t, Action):
                ...
                l = l[l.rindex('\\\\')+2:-1]
                t = (l, t.vk.login, t.vk.my_id, t.vk.my_name, task[1])
                temp.append(t)
    ...
    @get_data
    async def add_akk(self, data: Data):
        return self.response(await VkSession(data.login, data.password, data.user_agent, data.proxy).auth())

    @get_data
    async def list_akk(self, data: Data):
        return self.response([[x.login, x.my_id, x.my_name]
                              for x in Global.AllVkSession.values()
                              if x.methods.working and x.is_auth])

    async def server_task(self, name: str, func, params: dict) -> bool:
        try:
